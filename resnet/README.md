...
<table>
  ...
  <thead>
    <tr>
      <th>逻辑阶段</th>
      <th>说明（该阶段在 ResNet 中承担的论证任务）</th>
      <th>中文提纲 + 英文原句拆解（括注为对应段落与句子）</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      ...
      <td>
        • 场景：深度 CNN 带来图像分类的系列突破，端到端多层结构自然整合低/中/高层特征与分类器（原文第 1 段第 1-2 句）
        ...
        • 动机：网络深度至关重要——ImageNet 领先结果均采用 16~30 层的"极深"模型，许多视觉任务同样受益（原文第 1 段第 3-4 句）
    ...
    <tr>
      <td rowspan="3"><strong>研究问题分析与属性解读</strong></td>
      <td><strong>现状分析（总述）</strong>：由背景自然提问，再"排除"已解决的旧障碍，把读者带到真正的问题面前。</td>
      <td>
        • 承上启下提问：学习更好的网络是否就像堆叠更多层一样简单？<br>
        <blockquote>Driven by the significance of</blockquote>
        • 障碍铺垫：梯度消失/爆炸——已被归一化初始化 + 中间归一化层（BN）基本解决，数十层网络可经 SGD+BP 收敛（<strong>注意：这是"已解决"的旧障碍，写作上只起铺垫作用，不是本文的 Limitation</strong>）（原文第 2 段）
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        • 关键否定：<strong>不是过拟合</strong>——给适度深的模型再加层，<strong>训练误差</strong>反而升高（[10,41] 报告 + 本文 Fig.1 验证）（原文第 3 段）
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        • 结论：退化本质是<strong>优化困难</strong>——现有求解器找不到与构造解相当或更好的解（原文第 4 段）
      </td>
    </tr>
    <tr>
      ...
      <td>
        • 目标重构：不让堆叠层直接拟合目标底层映射 H(x)，改为显式拟合<strong>残差映射</strong> F(x) = H(x) − x，原始映射重构为 F(x) + x（原文第 5 段）
      </td>
    </tr>
    <tr>
      ...
      <td>
        • Goal 一句话（Technique paper 的目标只需一句交代）：提出深度残差学习框架以解决退化问题（原文第 5 段第 1 句）
        <blockquote>In this paper, we address the degradation problem by introducing a deep residual learning framework.</blockquote>
        • 核心假设（Hypothesis）：残差映射比原始、无参照的映射<strong>更易优化</strong>（原文第 5 段第 5 句；第 2-4 句的公式化定义 F(x) = H(x) − x 归入上一行"目标重构"）
        ...
        • 极端情形论证：若恒等映射即最优，把残差推向零比用非线性层堆叠拟合恒等映射更容易（原文第 5 段第 6 句）
    ...
    <tr>
      ...
      <td>ResNet 未显式罗列"3 个挑战"，非平凡性论证分两半完成：前一半在"属性解读"（构造解论证表明直接加深不可行），后一半在"方法总览"（实现零开销、训练机制不变）。对应模板中"投稿 AI 的会议经常不讨论技术挑战，而是直接讲 Our Methodology"的经典写法。</td>
      <td>
    ...
    <tr>
      ...
      <td>
        • F(x) + x 可由带 "shortcut connections" 的前馈网络实现（Fig.2）（原文第 6 段）
      </td>
    </tr>
    <tr>
      <td>第一个技术点：恒等跳连的结构。</td>
      <td>
        • 恒等 shortcut connections——跳过一个或多个层，输出与堆叠层输出直接相加（原文第 6 段，Fig.2）
      </td>
    </tr>
    <tr>
      <td>第二个技术点：零开销。</td>
    ...
    <tr>
      ...
      <td>
        • 整网仍可端到端 SGD+BP 训练；用常用库（如 Caffe[19]）即可实现，<strong>无需修改求解器</strong>（原文第 6 段）
      </td>
    </tr>
    <tr>
      ...
      <td>
        • ImageNet 综合实验（2 点结论）：① 极深残差网<strong>易优化</strong>，plain 网加深训练误差升高；② 残差网从大深度获得<strong>精度增益</strong>，显著优于先前网络（原文第 7 段）<br>
        • 泛化性：CIFAR-10 上同样现象 → 优化困难与方法效果<strong>不是数据集特例</strong>；成功训练 100+ 层，并探索 1000+ 层（原文第 8 段）<br>
        • 结果与竞赛：152 层 = ImageNet 史上最深网络、复杂度仍低于 VGG；集成 top-5 error <strong>3.57%</strong>；获 ILSVRC 2015 分类冠军 + ImageNet 检测/定位、COCO 检测/分割冠军 → 残差学习原理<strong>通用</strong>，预期适用于其他视觉与非视觉问题（原文第 9 段）
      </td>
    </tr>
  </tbody>
</table>
